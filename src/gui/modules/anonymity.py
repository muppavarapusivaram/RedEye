...
@dataclass
class ProxyEntry:
    """Represents a single proxychains entry."""
...
class ProxyConfiguration:
    """Aggregated proxychains configuration."""

    chain_mode: str
    entries: List[ProxyEntry]


SCRIPT_DIR = Path(__file__).resolve().parents[2] / "scripts" / "anonymity"


class ProxyConfigDialog(QDialog):
    """Gather SOCKS proxy settings from the user."""
...
class AnonymityModule(QWidget):
    """Interactive controls for proxychains, TOR, and OpenVPN management."""
    ...
    def _start_openvpn_process(self, profile_path: Path) -> None:
        ...
        if os.geteuid() == 0:
            ...
            if not password:
                ...
                return
            program = "sudo"
            arguments = ["-S", str(script_path), str(profile_path)]
    ...
    def shutdown(self) -> None:
        """Terminate any long-running processes before the application closes."""
        if self.openvpn_process and self.openvpn_process.state() == QProcess.Running:
            self.openvpn_process.terminate()
            self.openvpn_process.waitForFinished(2000)
            self.openvpn_process = None
        # Don't run sudo commands on shutdown - let user handle cleanup manually
